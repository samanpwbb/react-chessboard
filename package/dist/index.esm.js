...
import React__default, { createContext, memo, useEffect, isValidElement, cloneElement, createRef, Component, useLayoutEffect as useLayoutEffect$1, useMemo, useContext, useState, useCallback, useRef, forwardRef } from 'react';
...
const withAnimated = (Component, host) => {
  ...
  return /*#__PURE__*/forwardRef((givenProps, givenRef) => {
    ...
    return /*#__PURE__*/React.createElement(Component, _extends$1({}, usedProps, {
      ref: ref
    }));
  });
};
...
const ChessboardContext = /*#__PURE__*/React__default.createContext();
...
const ChessboardProvider = /*#__PURE__*/forwardRef(({
  ...
  const [positionDifferences, setPositionDifferences] = useState({}); // chess pieces/styling

  const chessPieces = useMemo(() => ({ ...defaultPieces,
    ...customPieces
  }), [defaultPieces, customPieces]); // whether the last move was a manual drop or not

  const [manualDrop, setManualDrop] = useState(false); // if currently waiting for an animation to finish

  const [transitioning, setTransitioning] = useState(false);
  const completeMove = useCallback(position => {
...
function Board() {
  ...
  return boardWidth ? /*#__PURE__*/jsx("div", {
    ref: boardRef,
    style: {
      position: 'relative'
    },
